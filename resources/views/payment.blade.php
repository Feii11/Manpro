...
<div class="container mx-auto p-6">
    ...
    <div class="bg-white p-6 shadow-lg rounded-lg">
        ...
        <form action="{{ route('payment.submit', ['order' => $order->id_order]) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <input type="file" name="payment_proof" class="block w-full text-sm text-gray-500" required>
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Submit Payment Proof</button>
        </form>
    </div>
</div>
